function zang2,angsiz,z, h0=h0, k = k, Lambda0 = lambda0, Omega_m = Omega_m, $
                    q0 = q0, SILENT = silent
;+
; NAME:
;       ZANG2
; PURPOSE:
;       Determine the linear scale of an object of a given angular size as 
;       a function of redshift
; EXPLANATION:
;       Requires an input angular size in arcsec and returns the size kpc.  
...
;
; CALLING SEQUENCE:
;       angsiz = zang2( angsiz, [ z, H0 =, Omega_m =, Lambda0 = , q0 = , k =, 
;                               /SILENT] )
;
; INPUTS:
;       angsiz - angular size  *in arcsec*, non-negative scalar or vector
...
; Angular distance is equal to the luminosity distance times (1+z)^2
 return,angsiz/(1.+z)^2*(1000.*d)/(!RADEG*3600.)
 end
